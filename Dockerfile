...
RUN cd freesasa-2.0.3 && ./configure --disable-threads --disable-xml --disable-json && make all && cd ../
ADD https://api.github.com/repos/blacktanktop/term_peptide_design/git/refs/heads/master version.json
RUN git clone https://github.com/blacktanktop/term_peptide_design.git
RUN cd term_peptide_design && make all && cd ../
